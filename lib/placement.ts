...
export default function (
  ...
  const margin =
    parseInt(overlayStyle[primary.marginBefore]) +
    parseInt(overlayStyle[primary.marginAfter]);
  const roomBefore =
    anchorRect[primary.before] - boundRect[primary.before] - margin;
  const roomAfter =
    boundRect[primary.after] - anchorRect[primary.after] - margin;

  if (
    (side === primary.before && overlay[primary.offsetSize] > roomBefore) ||
  ...
  if (side === primary.before) {
    ...
    // bottom or right
    overlay.style[primary.before] =
      scrollOffset + boundPrimaryPos(anchorRect[primary.after]) + "px";
    overlay.style[primary.after] = "auto";
  }
  ...
  switch (align) {
    ...
    default:
      // center
      const anchorSize =
        anchorRect[secondary.after] - anchorRect[secondary.before];
      overlay.style[secondary.before] =
        secondaryScrollOffset +
        boundSecondaryPos(
