...
export async function markdownToHtml(markdown) {
  ...
  const [contentHTML, excerptHTML] = [
    remarkInstance,
    excerptInstance,
  ].map(instance =>
    instance.use(highlight).use(externalLinks).use(html).process(markdown)
  );

  const excerptText = remark().use(excerpt).use(strip).process(markdown);

  return {
    contentHTML: (await contentHTML).toString(),
    excerptHTML: (await excerptHTML).toString(),
    excerpt: (await excerptText).toString(),
  };
}
